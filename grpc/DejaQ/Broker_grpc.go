...
func (c *brokerClient) TimelineConsume(ctx context.Context, in *flatbuffers.Builder, 
	opts... grpc.CallOption) (Broker_TimelineConsumeClient, error) {
...
func _Broker_TimelineConsume_Handler(srv interface{}, stream grpc.ServerStream) error {
  m := new(TimelineConsumeRequest)
  if err := stream.RecvMsg(m); err != nil { return err }
  return srv.(BrokerServer).TimelineConsume(m, &brokerTimelineConsumeServer{stream})
}
