...
func (c *Loader) loadMessages(ctx context.Context) bool {
	...
	hydratingConsumersAndPipelines := c.greeter.GetAllCustomersWithHydrateStatus(protocol.Hydration_Requested)
	activeConsumersAndPipelines := c.greeter.GetAllCustomersWithHydrateStatus(protocol.Hydration_Done)
...
func (c *Loader) hydrateOneConsumer(ctx context.Context, consumer *Consumer, consumerPipeline chan<- timeline.PushLeases) (int, error) {
	return 0, nil
}
