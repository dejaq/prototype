...
func (m *InMemory) Insert(ctx context.Context, timelineID []byte, msgs []timeline.Message) []storage.MsgErr {
...
func (m *InMemory) Update(ctx context.Context, timelineID []byte, messageTimestamps []storage.MsgTime) []storage.MsgErr { return nil }

//LOOKUP message by messageID (owner control, lease operations)
func (m *InMemory) Lookup(ctx context.Context, timelineID []byte, messageIDs [][]byte) ([]timeline.Message, []storage.MsgErr) {
	return nil, nil
}
...
func (m *InMemory) CountByRangeProcessing(ctx context.Context, timelineID []byte, a, b uint64) uint64 { return 0 }

//COUNT messages BY RANGE and LockConsumerID is not empty (count waiting status)
func (m *InMemory) CountByRangeWaiting(ctx context.Context, timelineID []byte, a, b uint64) uint64 { return 0 }

//SELECT messages by LockConsumerID (when consumer restarts)
func (m *InMemory) SelectByConsumer(ctx context.Context, timelineID []byte, consumerID []byte) []timeline.Message { return nil }

//SELECT messages by ProducerOwnerID (ownership control)
func (m *InMemory) SelectByProducer(ctx context.Context, timelineID []byte, producerID []byte) []timeline.Message { return nil }
